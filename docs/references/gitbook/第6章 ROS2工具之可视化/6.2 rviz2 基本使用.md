...
**rviz2 启动之后，默认界面如下：**![6.2rviz2界面简介](assets/6.2rviz2%E7%95%8C%E9%9D%A2%E7%AE%80%E4%BB%8B.PNG)

1.  上部为工具栏：包括视角控制、预估位姿设置、目标设置等，还可以添加自定义插件；
1.  左侧为插件显示区：包括添加、删除、复制、重命名插件，显示插件，以及设置插件属性等功能；
1.  中间为3D试图显示区：以可视化的方式显示添加的插件信息；
1.  右侧为观测视角设置区：可以设置不同的观测视角；
1.  下侧为时间显示区：包括系统时间和ROS时间。

**左侧插件显示区默认有两个插件：**

![6.2rviz2界面左侧](assets/6.2rviz2%E7%95%8C%E9%9D%A2%E5%B7%A6%E4%BE%A7.PNG)
...
下图是关于 rviz2 插件使用的示例，在该示例中 rviz2 集成了 TF、Laser Scan、Image 等插件，通过这些插件我们可以将一些肉眼不可见的数据以可视化的方式展现出来，以机器人的视角来看世界。![6.2rviz2基本使用示例](assets/6.2rviz2%E5%9F%BA%E6%9C%AC%E4%BD%BF%E7%94%A8%E7%A4%BA%E4%BE%8B.png)
