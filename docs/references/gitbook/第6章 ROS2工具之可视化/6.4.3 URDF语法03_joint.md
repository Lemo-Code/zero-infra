...
![6.4.3urdf_joint](assets/6.4.3urdf_joint.png)
...
-  **\<parent\>**（必填）：指定父级link。
    -  link
        （必填）：父级link的名字，是这个link在机器人结构树中的名字。
-  **\<child\>**（必填）：指定子级link。
    -  link
        （必填）：子级link的名字，是这个link在机器人结构树中的名字。
...
![6.4.3urdf_joint](assets/6.4.3urdf_joint.gif)
...
![6.4.3urdf_basefootprint](assets/6.4.3urdf_basefootprint.gif)
